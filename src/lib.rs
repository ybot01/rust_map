...
impl<const N: usize, V> ByteArrayTreeMap<N, V>{
    ...
    /*const*/ fn _contains(&self, key: &[u8;N], depth: usize) -> bool{
        match self{
            Self::Branch(branch) => branch[get_index(key, depth)]._contains(key, depth + 1),
    ...
    pub /*const*/ fn get_min_key(&self, exclude: &Self) -> Option<[u8; N]> {
        match self{
    ...
    pub /*const*/ fn get_max_key(&self, exclude: &Self) -> Option<[u8; N]> {
        match self{
    ...
    pub /*const*/ fn max_depth(&self, exclude: &Self) -> u64 {
        match self{
    ...
    /*const*/ fn _highest_shared_leading_zeroes_by_key(&self, key: &[u8; N], exclude: &Self, depth: usize) -> Option<[u8; N]> {
        match self{
            Self::Branch(branch) => {
                //const KEY_ORDER: [[usize; 4]; 4] = [[0,1,2,3], [1,0,2,3], [2,3,0,1], [3,2,0,1]];
                const KEY_ORDER: [[usize; 2]; 2] = [[0,1], [1,0]];
                KEY_ORDER[get_index(key, depth)].into_iter()
    ...
    /*const*/ fn _closest_by_abs_distance_by_key(&self, key: &[u8; N], exclude: &Self, depth: usize) -> (Option<([u8;N], [u8;N])>, bool, bool) {
        match self{
            Self::Branch(branch) => {
    ...
    /*const*/ fn _get(&self, key: &[u8;N], depth: usize) -> Option<&V>{
        match self{
            Self::Branch(branch) => branch[get_index(key, depth)]._get(key, depth + 1),
            Self::Leaf(None) => None,
...
impl<const N: usize, V: Clone> ByteArrayTreeMap<N, V>{
    fn _insert_or_update_if(&mut self, key: &[u8; N], value: V, condition: &impl Fn(&V) -> bool, depth: usize) -> bool {
        match self{
            Self::Branch(branch) => branch[get_index(key, depth)]._insert_or_update_if(key, value, condition, depth + 1),
            Self::Leaf(leaf) => {
                match leaf{
    ...
    fn _remove_if(&mut self, key: &[u8; N], condition: &impl Fn(&V) -> bool, depth: usize) -> bool{
        match self{
            Self::Branch(branch) => {
                if !branch[get_index(key, depth)]._remove_if(key, condition, depth + 1) {return false}
